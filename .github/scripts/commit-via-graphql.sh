...
# which (unlike GITHUB_TOKEN) re-triggers downstream workflows.
#
# Required env:
#   GH_TOKEN     installation token (gh CLI uses this automatically)
#   REPO         owner/name (e.g. hayatosc/dotfiles)
...
while :; do
  ...
  # tree mode が 100644 で固定される一方、再生成コマンドは fs に +x を付けうる。
  # fs vs index の mode 差を「変更あり」と誤認すると空 commit を打ち続けるので無視する。
