...
- **時間予算**: 1 iteration（シナリオ 2 本）は実行者の能力・effort によって数分〜15 分超まで振れる。実行環境にターンや CLI のタイムアウトがある場合は、シナリオを小さくする（成果物 1 つ・要件 3 〜 5 項目）か iteration 単位で区切って途中結果を報告する。1 ターンに収まらない設計は、タイムアウトで評価結果ごと失われる
...
- **シナリオの前提環境を用意していない**: 対象プロンプトが前提とする環境（git リポジトリ、依存ツール、入力ファイル）を fixture に揃えてから回す。前提が欠けると実行者は環境探索に時間を溶かし、プロンプトではなく環境の欠陥を測ってしまう
